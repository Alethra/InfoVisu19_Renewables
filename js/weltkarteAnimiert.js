...
async function setup() {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    circle = {
      ...
      //CS also keep track of the data corresponding to the circle
      d: d
    };
...
function draw() {
  ...
  text("Jahr: " + jahr, 100, 100);

  //CS mainly update radius and nr of rings
  updateCircles();

  //CS recalculate circle positions to avoid circle overlaps
  resolveOverlaps();
...
function updateCircles() {
  ...
  for (var i = 0; i < circles.length; i++) {
    ...
    //recalculate number of rings based on current year
    circle.rings = Math.floor(circle.d[yearKey] / 5);

    //recalculation maxDiameter basde on current year
    circle.maxDiameter = 10 * Math.floor(circle.d[yearKey] / 5);
  }
...
function resolveOverlaps() {
  ...
  //CS reset circle vectors to initial vectors
  for (var i = 0; i < circles.length; i++) {
    var circle = circles[i];
    circle.vec = createVector(lonScale(circle.d.Lon), latScale(circle.d.Lat))
  }
